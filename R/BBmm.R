BBmm <- function(fixed.formula=NULL, X=NULL,y=NULL,random.formula=NULL,Z=NULL,nRandComp=NULL,m,data=list(),method="BB-Delta",maxiter=50,show=FALSE,nDim=1,tolerance=10^(-6)){
  ...
  while(tol>tolerance){
    ...
    if (method=="BB-Delta"){
      rand.fix <- EffectsEst.BBDelta(y,m.,beta,u,phi,D.,X,Z,maxiter,nDim)
      if (rand.fix$conv=="no"){
        beta <- rand.fix$beta
        u <- rand.fix$u
    ...
    ####
     if (show==TRUE){
       ...
       cat("Tolerance:", tol,"\n")
       cat("Beta difference:", max(abs(beta-OLDbeta)/OLDbeta),"\n")
     }
    ####
    ...
    ####
    if(show==TRUE){
      cat("Phi difference:", max(abs(phi-OLDphi)/OLDphi),"\n")
      cat("Sigma difference:", max(abs(all.sigma-OLDall.sigma)/OLDall.sigma),"\n")
      cat("Sigma:",all.sigma,"\n")
      cat("phi:",phi,"\n")
